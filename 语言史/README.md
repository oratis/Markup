...
**2. 欠账 762 条：已清 754（99%），待核 8，成书前 0。**

跑 `python3 tools/debt_report.py` 看分章明细。**成书前一栏是零**：那批「定稿时重取」的数字已在 2026-08-04 取到一手数据。**待核 8 条全是 C 型**（第 8 章 2、第 9 章 5、第 15 章 1）：各条都写明了试过的检索路线，卡在纸本或不可解析的扫描件上（《语言问题》讲次页码、周有光「四成」的原刊页码、台港识字率、文特里斯 1936 年展览的具体日期等）。早先的报告把它们数成了 0——它们是清账时从父条目裂出来的缩进子条目，旧版 `debt_report.py` 只数行首的 `- [ ]`；工具已改，现在连缩进的一并计入。
