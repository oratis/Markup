#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""把 语言史/ 下的稿件编译成一份自包含的单文件 HTML 阅读稿。

用法（工作目录必须是 语言史/）：
    python3 tools/build_html.py [输出路径]

默认输出到 ../星号之下-全书.html（不放进仓库，1MB 量级的生成物不该进 git）。
...
def collect():
    ...
    items.append(("ch-0", "front", "序", "序章 · 绵羊和马", "序章-绵羊和马.md"))
    eras = [
        ("第一卷-开口", "第一卷 · 开口"),
        ("第二卷-刻下", "第二卷 · 刻下"),
        ("第三卷-认亲", "第三卷 · 认亲"),
    ...
    items.append(("ch-99", "front", "终", "终章 · 星号的另一边", "终章-星号的另一边.md"))
    for p, label in [("提纲/全书提纲.md", "全书提纲"), ("提纲/语系谱.md", "语系谱"),
                     ("提纲/写作规范.md", "写作规范"),
...
def md_to_html(md, linkmap):
    ...
    while i < len(lines):
        ...
        if st.startswith(">"):
            ...
            cls = "correction" if "订正" in first else "quote"
            if cls == "correction":
        ...
        if buf:
            out.append(f'<p>{inline(" ".join(buf), linkmap)}</p>')
    return "".join(out)
...
<script>
(function(){
  var H=document.documentElement, K="yyg:";
...
if __name__ == "__main__":
    out = sys.argv[1] if len(sys.argv) > 1 else "../星号之下-全书.html"
    build(out)
