...
search that indexes 10,000 files in about a second. Free, no account.
macOS-only, pre-1.0.
```
...
⌘/ for source). It's a Tauri app, so it stays light (~88MB idle).
...
It reads your .md like a web page, and you edit on demand. Tauri-light
(~88MB), vault + backlinks + graph + full-text search, MIT.
